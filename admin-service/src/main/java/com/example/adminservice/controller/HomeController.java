...
adminAccess.getTrainDetails().add(trainDetails);
    ...
    public ResponseEntity<Void> updateTrainDetails(@RequestBody TrainDetails trainDetails,@PathVariable("trainNumber") String trainNumber){
        if(trainDetails==null){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        }else {
            this.trainService.updateTrainDetails(trainDetails, trainNumber);
            return ResponseEntity.status(HttpStatus.OK).build();
        }
    }

    @RequestMapping(value = "/delete-TrainDetails/{trainNumber}",method = {RequestMethod.DELETE,RequestMethod.GET})
    public ResponseEntity<Void> deleteTrainDetails(@PathVariable("trainNumber")String trainNumber){
        if(trainNumber==null){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        }else{
            this.trainService.deleteTrainDetails(trainNumber);
            return ResponseEntity.status(HttpStatus.OK).build();
        }
    }
